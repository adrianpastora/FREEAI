...
class Settings(BaseSettings):
    ...
    # When true, the setup wizard requires the operator to manually copy the
    # bootstrap token AND the master encryption key from the server logs into
    # the UI form. This is the right default for instances directly exposed to
    # the public internet from the very first request.
    #
    # When false (default), the master key is autogenerated and silently
    # promoted on first cold start (no manual paste), and the bootstrap token
    # is auto-fetched by the frontend when the request comes from loopback
    # (127.0.0.1 / ::1). The token still exists and is still required on the
    # POST that creates the first admin — we just save the operator from
    # copying it by hand on the LAN install path.
    require_bootstrap_header: bool = Field(
        default=False,
        description=(
            "If true, the master key + bootstrap token must be pasted into "
            "the UI manually (paranoid mode for instances exposed to the "
            "internet on first boot). Default false: localhost-friendly "
            "auto-confirmation."
        ),
    )
