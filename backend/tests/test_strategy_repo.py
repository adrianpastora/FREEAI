...
async def test_create_custom_strategy(session):
    repo = StrategyRepository(session)
    dto = StrategyDTO(name="my_strategy", tags=["fast", "coding"], description="mine", is_builtin=False)
    await repo.upsert(dto)
    await session.commit()
    found = await repo.get("my_strategy")
    assert found is not None
    assert found.tags == ["fast", "coding"]
...
async def test_can_delete_custom(session):
    repo = StrategyRepository(session)
    await repo.upsert(StrategyDTO(name="temp", tags=[], description="", is_builtin=False))
    await session.commit()
...
async def test_upsert_preserves_builtin_flag(session):
    ...
    repo = StrategyRepository(session)
    # seed
    await repo.upsert(StrategyDTO(name="coding", tags=["coding"], description="orig", is_builtin=True))
    await session.commit()
    # update — our SQL excludes is_builtin from the SET clause
    await repo.upsert(StrategyDTO(name="coding", tags=["coding", "reasoning"], description="new", is_builtin=False))
    await session.commit()
    found = await repo.get("coding")
    assert found.is_builtin is True
    assert found.tags == ["coding", "reasoning"]
